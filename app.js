// alemty.eth v0.06 — fixes: drawer scroll stable, POAP crop fix, forum search, JS sanitization.
...
function getAuraBalance(addr){
  // Aura / ALEM balance placeholder (hasta integrar token on-chain)
  const a = String(addr||'').toLowerCase();
  const k = `aura.balance.${a}`;
  const v = localStorage.getItem(k);
  const n = Number(v);
  return Number.isFinite(n) ? Math.max(0, n) : 0;
}
function getAlemBalance(addr){
  // ALEM token placeholder (hasta integrar RPC/contract)
  const a = String(addr||'').toLowerCase();
  const k = `alem.balance.${a}`;
  const v = localStorage.getItem(k);
  const n = Number(v);
  return Number.isFinite(n) ? Math.max(0, n) : 0;
}
function openCharacterModal(){
  ...
  title.textContent = 'Personaje · ' + shortAddr(addr);
  ...
  body.innerHTML = `
    <div class="char-sheet">
      <div class="char-card">
        <div class="char-avatar" aria-label="Avatar">
          <div class="char-halo" aria-hidden="true"></div>
          <img src="${escapeHtml(document.getElementById('avatarImg')?.getAttribute('src') || '20260223_205715.jpg')}" alt="Avatar" loading="eager"/>
        </div>
        
<div class="char-role">${escapeHtml(DID.role || 'Miembro')}</div>
<h3 class="char-title">${escapeHtml(rank)}</h3>
<p class="meta small mono addr">${escapeHtml(shortAddr(addr))}</p>
<p class="meta small">Siguiente: <strong>${escapeHtml(next)}</strong></p>


        <div class="equip-grid" aria-label="Equipo">
          <div class="equip-slot">
            <div class="lbl">Medalla rango</div>
            <div class="ico">🏅</div>
            <div class="val">${escapeHtml(rank)}</div>
          </div>
          <div class="equip-slot">
            <div class="lbl">Rol</div>
            <div class="ico">🛡️</div>
            <div class="val">${escapeHtml(DID.role || 'Miembro')}</div>
          </div>
          <div class="equip-slot">
            <div class="lbl">veNFT</div>
            <div class="ico">⛓️</div>
            <div class="val">—</div>
          </div>
          <div class="equip-slot">
            <div class="lbl">Agente IA</div>
            <div class="ico">🤖</div>
            <div class="val">—</div>
          </div>
          <div class="equip-slot">
        ...
        <div class="bars">
          <div class="bar">
            <div class="bar-top">
              ...
              <div class="meta small">Progreso: <strong>${escapeHtml(s.progress)}%</strong></div>
            </div>
            <div class="bar-track"><div class="bar-fill" style="width:${s.progress}%"></div></div>
          </div>

          <div class="bar">
            <div class="bar-top">
              <div><strong>Karma</strong> <span class="meta small">${escapeHtml(s.karma)}</span></div>
              <div class="meta small">(moderación / reputación)</div>
            </div>
            <div class="bar-track"><div class="bar-fill karma" style="width:${Math.min(100, Math.max(5, Math.round((s.karma/200)*100)))}%"></div></div>
          </div>

          <div class="bar">
            <div class="bar-top">
              <div><strong>Aura</strong> <span class="meta small">${escapeHtml(aura)}</span></div>
              <div class="meta small">(valor interno)</div>
            </div>
            <div class="bar-track"><div class="bar-fill aura" style="width:${Math.min(100, Math.max(6, Math.round((aura/500)*100)))}%"></div></div>
          </div>

          <div class="profile-grid">
            <div class="stat"><div class="k">ALEM</div><div class="v">${escapeHtml(alem)}</div></div>
          ...
          <p class="meta small" style="margin-top:10px">Nota: Aura/ALEM se muestran como placeholder local hasta integrar contrato en Base.</p>
        </div>
      </div>
    </div>
  `;
  ...
  const closeFn = () => { modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); };
  document.getElementById('characterClose')?.addEventListener('click', closeFn, { once:true });
  document.getElementById('characterBackdrop')?.addEventListener('click', closeFn, { once:true });
}
function bindCharacterUI(){
  document.getElementById('profileBtn')?.addEventListener('click', openCharacterModal);
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      const m = document.getElementById('characterModal');
      if (m?.classList.contains('open')){ m.classList.remove('open'); m.setAttribute('aria-hidden','true'); }
    }
  });
}
...
function syncWalletUI() {
  ...
  if (DID.connected && DID.address) {
    ...
    if (loginHint) loginHint.hidden = true;   // ✅ OCULTAR TEXTO
  } else {
    connectWalletBtn.hidden = false;
    disconnectWalletBtn.hidden = true;
    setWalletStatus('No conectado', 'muted');

    if (loginHint) loginHint.hidden = false;  // ✅ MOSTRAR TEXTO
  }
}
...
function renderDidCard() {
  ...
  if (status) {
    status.textContent = DID.connected
      ? 'Miembro (DID verificado)'
      : 'Visitante (solo lectura)';
  }

  if (addr) {
    addr.textContent = DID.connected && DID.address
      ? shortAddr(DID.address)
      : '—';
  }

  if (role) {
    role.textContent = DID.connected
      ? DID.role ?? 'Miembro'
      : '—';
  }
}
...
function setView(which) {
  if (which === 'comunidad') {
    ...
    document.title = 'comunidad.alemty.eth · v0.06';
  } else {
    pageIdentity.hidden = false;
    pageCommunity.hidden = true;
    document.title = 'alemty.eth v0.06';
  }
